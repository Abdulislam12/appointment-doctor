...
const createSlots = async (req, res, next) => {
    ...
    try {
        ...
        if (extraFields.length > 0) {
            return res.status(400).json(new ApiResponse(400, `Unexpected fields: ${extraFields.join(", ")}`));
        }
...
const getDoctorAppointmentsWithFilter = async (req, res, next) => {
    try {
        ...
        const appointments = await getDoctorAppointmentsFilterService(doctorId, date, AppointmentStatus);
